...
echo -e "\n🏹 =============================================== 🏹"
echo -e "🏹 SWISS DEMO: n8n Workflow Test Number 1 via localhost:5678 (POST)"
echo -e "🏹 ----- Starting n8n Sanity Check ------------------"
n8n_URL_WEBHOOK_REQUEST="http://localhost:5678/webhook-test/sanity-check-test"
n8n_WEBHOOK_RESPONSE=$(curl -sk -X POST $n8n_URL_WEBHOOK_REQUEST \
  -H "Content-Type: application/json" \
  -d '{"test":"sanity"}' | jq .)
echo "$n8n_WEBHOOK_RESPONSE"
echo -e "🏹 ---- Completed sanity check -------------"
# Swiss Demo: n8n Workflow Test (POST)
echo -e "\n🏹 SWISS DEMO: n8n Workflow Test via n8n.helix.local (POST)"
echo "------------------------------------------------------"
curl -sk -X POST https://n8n.helix.local:8443/webhook/sanity-check-prod \
  -H "Content-Type: application/json" \
  -d '{"test":"sanity"}'
echo "------------------------------------------------------"
echo -e "\n🏔️ HELIX HUB SWISS CURL ENDPOINT DEMO"
echo -e "🎯 All-in-one: Service, Workflow, and AI Checks - so it better work now folks!"
echo -e "===============================================\n"
...
for variant in "${!CHUCK_VARIATIONS[@]}"; do
  ...
  echo -e "🏹 ------------------------------------------------------"
  curl -sk -X POST https://n8n.helix.local:8443/webhook/sanity-check \
    -H "Content-Type: application/json" \
